import { 
  ...
  export class TransactionsController {
    ...
    @ApiResponse({ status: HttpStatus.CONFLICT, description: 'Transaction reference already exists' })
    async create(@Body() createTransactionDto: CreateTransactionDto, @Request() req): Promise<Transaction> {
      ...
      if (!createTransactionDto.reference) {
        createTransactionDto.reference = await this.transactionsService.generateUniqueReference();
      }
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Transaction details', type: Transaction })
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Transaction details', type: Transaction })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Transaction not found' })
    async findByReference(@Param('reference') reference: string, @Request() req): Promise<Transaction> {
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Transaction updated successfully', type: Transaction })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Transaction not found' })
    @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Access denied' })
    @ApiResponse({ status: HttpStatus.CONFLICT, description: 'Transaction reference already exists' })
    async update(
      ...
      if (updateTransactionDto.userId && updateTransactionDto.userId !== req.user.id) {
